...
- Failed structured-output attempts fall back once instead of exhausting the
  full schema retry budget, so a mismatched verifier model costs far less time
  per run.
